...
def main():
    ...
    if st.button("Predict"):
        ...
        st.table(results.style.background_gradient(cmap=cm))
        ...
        ###############################################
        st.text("")
        st.subheader(":red[Forecast for the next 222 days]", anchor=None)
        
        forecast = pd.DataFrame(model.predict(start=data.index[-1] + 1, end=data.index[-1] + 12))
        st.table(forecast.style.background_gradient(cmap=cm))
